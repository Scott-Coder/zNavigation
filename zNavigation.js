...
(function(window,undefined){
		...
		build : function(data){
			...
			$(data).each(function(i,v){
				if(v.children==undefined){
					result += "<li data-content=\""+v.name+"\"><a "+((v.url!=undefined)?"url=\""+v.url+"\"":"")+((v.id!=undefined&&v.id!="")?" id=\""+v.id+"\"":"")+">"+((n.get("icon")==true&&v.ico!=undefined)?"<span class=\"nav-icon\"><img src=\""+((v.ico!=undefined)?v.ico:"")+"\"></span>":"")+v.name+"</a></li>";
				}else{
					result += "<li data-content=\""+v.name+"\"><a "+((v.url!=undefined)?"url=\""+v.url+"\"":"")+((v.id!=undefined&&v.id!="")?" id=\""+v.id+"\"":"")+">"+((n.get("icon")==true&&v.ico!=undefined)?"<span class=\"nav-icon\"><img src=\""+((v.ico!=undefined)?v.ico:"")+"\"></span>":"")+v.name+"<i class=\"fa fa-chevron-down\"></i></a><ul class=\"submenu\">"+n.build(v.children)+"</ul></li>";
				}
			});
			return result;
		},
			...
			n.get("ele").off("click","ul li").on("click","ul li",function(){
                ...
                if ($(this).children(".submenu").length > 0) {
					...
					var data = {
						...
						icon : $(this).find(">a>.nav-icon img").attr("src")
					}
					n.get("onClick")($(this),data);
					return false;
				}
			}).on("click",".closeBtn",function(){
				n.get("ele").toggleClass("close");
			})/*.on("mouseover","ul.zNavigation-list>li",function(){
				console.log(0);
	...
	$.fn.zNavigation = function(option){
		option.ele = $(this);
		$(this).data(new zNavigation(option));
	}
